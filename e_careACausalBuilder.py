...
class ECARECausalBuilder:
    ...
    def _normalize_choice_text(text: Any) -> str:
        ...
        if s in {"no effect", "no-effect", "no_effect", "noeffect"}:
            return "no_effect"
        if s in {"more"}:
            return "more"
        if s in {"less"}:
    ...
    @classmethod
    def _normalize_datapoint(cls, datapoint: Dict[str, Any]) -> Dict[str, Any]:
        """
        Normalize different dataset formats into a WIQA-like datapoint schema.

        Supported:
        - WIQA-style dict: has 'question_stem' and 'answer_label'.
        - e-care 100_more_less_noeffect_simple JSONL: has
          'question', 'choices' (list), 'label_id/label_text', 'A', 'B', 'rationale'.
        """
        dp: Dict[str, Any] = dict(datapoint or {})

        # --- WIQA style ---
        if "question_stem" in dp and "answer_label" in dp:
            dp["answer_label"] = cls._normalize_choice_text(dp.get("answer_label"))
            choices = dp.get("choices")
            if isinstance(choices, list):
                texts = [cls._normalize_choice_text(x) for x in choices]
                labels = [chr(ord("A") + i) for i in range(len(texts))]
                dp["choices"] = {"text": texts, "label": labels}
            elif not (isinstance(choices, dict) and "text" in choices and "label" in choices):
                dp["choices"] = {"text": ["more", "less", "no_effect"], "label": ["A", "B", "C"]}

            if not dp.get("answer_label_as_choice"):
                try:
                    idx = list(dp["choices"]["text"]).index(dp["answer_label"])
                    dp["answer_label_as_choice"] = dp["choices"]["label"][idx]
                except Exception:
                    dp["answer_label_as_choice"] = {"more": "A", "less": "B", "no_effect": "C"}.get(dp["answer_label"], "")
            return dp

        # --- e-care (100_more_less_noeffect_simple) style ---
        if "question" in dp and ("label_text" in dp or "label_id" in dp):
            dp.setdefault("question_stem", dp.get("question", ""))

            raw_choices = dp.get("choices") or ["More", "Less", "No effect"]
            if isinstance(raw_choices, dict) and "text" in raw_choices:
                texts = [cls._normalize_choice_text(x) for x in (raw_choices.get("text") or [])]
                labels = [str(x).strip().upper() for x in (raw_choices.get("label") or [])]
                if not labels:
                    labels = [chr(ord("A") + i) for i in range(len(texts))]
            else:
                texts = [cls._normalize_choice_text(x) for x in list(raw_choices)]
                labels = [chr(ord("A") + i) for i in range(len(texts))]
            dp["choices"] = {"text": texts, "label": labels}

            answer_label = ""
            if dp.get("label_text") is not None:
                answer_label = cls._normalize_choice_text(dp.get("label_text"))
            if not answer_label and dp.get("label_id") is not None:
                try:
                    label_id = int(dp.get("label_id"))
                except Exception:
                    label_id = -1
                if 0 <= label_id < len(texts):
                    answer_label = texts[label_id]
            dp["answer_label"] = answer_label or "unknown"

            if not dp.get("answer_label_as_choice"):
                label_id = dp.get("label_id")
                if label_id is not None:
                    ...
                    if 0 <= label_id < len(labels):
                        dp["answer_label_as_choice"] = labels[label_id]
                if not dp.get("answer_label_as_choice"):
                    try:
                        idx = texts.index(dp["answer_label"])
                        dp["answer_label_as_choice"] = labels[idx]
                    except Exception:
                        dp["answer_label_as_choice"] = {"more": "A", "less": "B", "no_effect": "C"}.get(dp["answer_label"], "")

            dp.setdefault("_source_format", dp.get("format", "e-care"))
            return dp
    ...
    def __init__(
        ...
        self.datapoint = self._normalize_datapoint(datapoint)
        self.model_name = model_name
        self.question = self.datapoint.get('question_stem', '') or self.datapoint.get("question", "")
        self.answers = self.datapoint.get('answer_label', "")
        # For e-care: "If A does NOT happen ..." cases
        self.cause_is_negated: bool = False
        # 各类节点集合（后续因果图构建时使用）
    ...
    def extract_start_entity(self) -> Optional[Dict[str, str]]:
        ...
        # =======================================================
        # IMPORTANT:
        # - Do NOT directly use datapoint["A"]/["B"] (treat them as absent).
        # - For e-care style questions:
        #     "If <A>, what happens to the chance that <B>?"
        #   we rewrite them into a WIQA-like "suppose ... affect ..." template,
        #   then reuse the same WIQA extraction prompt below.
        question_for_extraction = str(self.question or "").strip()
        self.cause_is_negated = False

        m_if = re.match(
            r"^\s*If\s+(?P<cause>.+?),\s*what happens to the chance that\s+(?P<outcome>.+?)\?\s*$",
            question_for_extraction,
            flags=re.IGNORECASE,
        )
        if m_if:
            cause_raw = str(m_if.group("cause") or "").strip()
            outcome_raw = str(m_if.group("outcome") or "").strip()

            neg_pat = re.compile(
                r"\bdoes\s+not\s+happen\b|\bdoesn't\s+happen\b|\bdo\s+not\s+happen\b|"
                r"\bdoes\s+not\s+occur\b|\bdoesn't\s+occur\b|\bnever\s+happen\b|\bnot\s+happen\b",
                re.IGNORECASE,
            )
            if neg_pat.search(cause_raw):
                self.cause_is_negated = True
                cause_positive = neg_pat.sub("", cause_raw).strip()
                cause_positive = re.sub(r"\s+", " ", cause_positive).strip()
            else:
                cause_positive = cause_raw
...
""".strip()
        ...
        raw_cause = re.sub(r"\b(happens|happen)\b\s*,?\s*$", "", raw_cause, flags=re.IGNORECASE).strip()
...
"""
    ...
    @staticmethod
    def _invert_effect(effect_on_base: Any) -> str:
        e = str(effect_on_base or "").strip().lower()
        if e == "more":
            return "less"
        if e == "less":
            return "more"
        if e in {"no_effect", "noeffect"}:
            return "no_effect"
        return "no_effect" if e in {"", "unknown", "none"} else e
...
""".strip()
    ...
    def reason_with_description(
        ...
        """
        original_question = question or self.question
        original_question = original_question or self.question

        # For e-care, some items ask: "If A does NOT happen ...".
        # We build the causal graph starting from A (positive event), so the
        # LLM aggregator should judge the effect of A happening first; then we
        # invert at the end to match the original question condition.
        question_for_llm = original_question
        if getattr(self, "cause_is_negated", False):
            m_if = re.match(
                r"^\s*If\s+(?P<cause>.+?),\s*what happens to the chance that\s+(?P<outcome>.+?)\?\s*$",
            ...
            if m_if:
                ...
                cause_positive = re.sub(r"\s+", " ", cause_positive).strip()
                question_for_llm = f"If {cause_positive}, what happens to the chance that {outcome_raw}?"
        ...
        if getattr(self, "cause_is_negated", False):
            final_effect = self._invert_effect(final_effect)
        ...
        cause_note = " | [CauseNegation] inverted for NOT(A) question" if getattr(self, "cause_is_negated", False) else ""
        combined_reason = f"[LLM] {reasoning} | [Graph] effect={graph_signal.get('graph_effect')} conf={graph_signal.get('graph_confidence')} | decision={decision_source}{cause_note}"
        ...
        graph_summary["original_question"] = original_question
        graph_summary["llm_question_used"] = question_for_llm
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run ECARECausalBuilder on e-care style JSONL.")
    parser.add_argument(
        "--input",
        type=str,
