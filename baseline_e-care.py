"""Converted from `baseline_e-care.ipynb`.

Runs CoT / CausalCoT / GoT / ToT baselines on the E-CARE more/less/no-effect dataset.
"""
...
SINGLE_ROW_LINE_NO: Optional[int] = None  # set to an int (>0) to select by JSONL line number
...
def load_rows(input_jsonl: str, *, max_samples: int = 0) -> List[Dict[str, Any]]:
...
def print_dataset_stats(rows: List[Dict[str, Any]]) -> None:
    print('rows:', len(rows))
...
def run_method(
    ...
    *,
    ...
    if method not in {'CoT', 'CausalCoT', 'GoT', 'ToT'}:
    ...
    # Avoid first-call races in threads (imports + monkey patches).
    if method in {'GoT', 'ToT'} or ollama is None:
        _ = ensure_cdcr_sft_ready()

    print(
        f'\n=== Running {method} on {len(indices)} rows '
        f'(resume: {len(processed_ids)} done, workers: {max_workers}) ==='
    )
    ...
    def _process_one(idx: int) -> Dict[str, Any]:
        ...
        if CACHE_BY_QUESTION:
            ...
            if cached is not None:
                ...
                if CACHE_BY_QUESTION and success:
                    with cache_lock:
    ...
    if max_workers <= 1 or len(todo_indices) <= 1:
        for local_i, idx in enumerate(todo_indices, start=1):
            result_row = _process_one(idx)
            upsert_row_by_id(out_csv, CSV_FIELDS, result_row)
            ...
            if len(indices) <= 3:
                ...
                if row.get('outcome_polarity') is not None:
                    meta = f" outcome_polarity={row.get('outcome_polarity')} base={row.get('answer_label_base')}"
                print(
                    f"[{method}] row_id={result_row['id']}{meta} gold={result_row['label']} "
                    f"pred={result_row['answer']} choice={result_row['letter_answer']}"
                )
                if method in {'GoT', 'ToT'}:
                    print(f"[{method}] LLM response:\n{result_row['llm_output']}")
                    ex = str(result_row.get('llm_extracted_output', '') or '').strip()
                    if ex:
                        print(f'[{method}] extracted: {ex}')
                    print('---')

            if local_i % 10 == 0:
                print(f'[{method}] processed {local_i}/{len(todo_indices)}')
    else:
        completed = 0
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {executor.submit(_process_one, idx): idx for idx in todo_indices}
            for fut in as_completed(futures):
                ...
                if completed % 10 == 0:
                    print(f'[{method}] processed {completed}/{len(todo_indices)}')
...
def select_single_index(rows):
    if SINGLE_ROW_LINE_NO is not None and int(SINGLE_ROW_LINE_NO) > 0:
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='E-CARE baseline runner (converted from baseline_e-care.ipynb)')
    p.add_argument('--input-jsonl', default=INPUT_JSONL)
    p.add_argument('--model', default=OLLAMA_MODEL)
    p.add_argument('--out-dir', default=OUT_DIR)
    p.add_argument('--methods', default=','.join(METHODS), help='Comma-separated, e.g. CoT,GoT,ToT,CausalCoT')
    p.add_argument('--mode', default=RUN_MODE, choices=['single', 'batch'])
    p.add_argument('--single-index', type=int, default=SINGLE_ROW_INDEX)
    p.add_argument('--single-line-no', type=int, default=(SINGLE_ROW_LINE_NO or 0))
    p.add_argument('--single-query', default=SINGLE_ROW_QUERY)
    p.add_argument('--max-samples', type=int, default=MAX_SAMPLES)
    p.add_argument('--seed', type=int, default=SEED)
    p.add_argument('--workers', type=int, default=max(1, min(8, (os.cpu_count() or 8))))
    p.add_argument('--cache-by-question', action=argparse.BooleanOptionalAction, default=CACHE_BY_QUESTION)
    p.add_argument('--rerun-errors', action=argparse.BooleanOptionalAction, default=RERUN_ERRORS)
    p.add_argument('--force-rerun-single', action='store_true', default=FORCE_RERUN_SINGLE)
    p.add_argument('--inspect-method', default='', help='If set, prints first N wrong rows for that method')
    p.add_argument('--inspect-n', type=int, default=10)
    return p.parse_args()


def main() -> int:
    global INPUT_JSONL, OLLAMA_MODEL, OUT_DIR, DATASET_NAME
    global RUN_MODE, SINGLE_ROW_INDEX, SINGLE_ROW_LINE_NO, SINGLE_ROW_QUERY
    global RERUN_ERRORS, FORCE_RERUN_SINGLE, METHODS, SEED, MAX_SAMPLES, CACHE_BY_QUESTION

    args = _parse_args()
    INPUT_JSONL = str(args.input_jsonl)
    OLLAMA_MODEL = str(args.model)
    OUT_DIR = str(args.out_dir)
    DATASET_NAME = os.path.splitext(os.path.basename(INPUT_JSONL))[0]

    RUN_MODE = str(args.mode)
    SINGLE_ROW_INDEX = int(args.single_index)
    SINGLE_ROW_LINE_NO = int(args.single_line_no) if int(args.single_line_no) > 0 else None
    SINGLE_ROW_QUERY = str(args.single_query)

    RERUN_ERRORS = bool(args.rerun_errors)
    FORCE_RERUN_SINGLE = bool(args.force_rerun_single)
    SEED = int(args.seed)
    MAX_SAMPLES = int(args.max_samples)
    CACHE_BY_QUESTION = bool(args.cache_by_question)

    METHODS = [m.strip() for m in str(args.methods).split(',') if m.strip()]

    random.seed(SEED)

    rows = load_rows(INPUT_JSONL, max_samples=MAX_SAMPLES)
    ...
    if mode.startswith('single'):
        ...
        for method in METHODS:
            out_csv = run_method(method, rows, row_indices=[idx], max_workers=int(args.workers))
            correct, total, acc = compute_accuracy(out_csv)
            results.append({'method': method, 'accuracy': acc, 'correct': correct, 'total': total, 'csv': out_csv})
    else:
        for method in METHODS:
            out_csv = run_method(method, rows, max_workers=int(args.workers))
            correct, total, acc = compute_accuracy(out_csv)
            results.append({'method': method, 'accuracy': acc, 'correct': correct, 'total': total, 'csv': out_csv})
    ...
    inspect_method = str(args.inspect_method).strip()
    if inspect_method:
        inspect_csv = get_output_csv_path(inspect_method)
        df_inspect = pd.read_csv(inspect_csv)
        wrong = df_inspect[df_inspect['is_correct'].astype(str).str.lower() == 'false']
        print(f'\n=== Wrong examples ({inspect_method}) ===')
        print(wrong.head(int(args.inspect_n)).to_string(index=False))
