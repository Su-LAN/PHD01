...
class DDXPlusCausalBuilder:
    ...
    def extract_start_entity(self) -> Optional[Dict[str, str]]:
        ...
        def _python_stage1_extract(question: str) -> Optional[Dict[str, str]]:
            """
            Deterministic extractor for common WIQA-style templates.

            Supports patterns like:
              - "Suppose ... , how will it affect ...?"
              - "If ... , how will this affect ...?"
            """
            q = (question or "").strip()
            if not q:
                return None

            m = re.search(r"\bhow\s+will\s+(?:this|it|that)\s+affect\b", q, flags=re.IGNORECASE)
            if not m:
                return None

            cause_part = q[: m.start()].strip()
            outcome_part = q[m.end() :].strip()

            cause_part = re.sub(r'^[\'"]+', "", cause_part).strip()
            outcome_part = re.sub(r"^[\s,:;\-\.]+", "", outcome_part).strip()

            # Strip leading markers in the cause clause.
            cause_part = re.sub(
                r"^\s*(suppose\s+that|suppose|if)\s+",
                "",
                cause_part,
                flags=re.IGNORECASE,
            ).strip()
            cause_part = cause_part.rstrip(" ,.;:")

            # Keep outcome text raw but remove trailing question mark.
            outcome_part = outcome_part.rstrip(" ?")

            # Direction words: strict whole-word matching.
            outcome_lower = outcome_part.lower()
            outcome_direction = "NONE"
            if re.search(r"\b(more|greater)\b", outcome_lower):
                outcome_direction = "MORE"
            elif re.search(r"\b(less|fewer)\b", outcome_lower):
                outcome_direction = "LESS"

            is_negated = bool(re.search(r"\b(not|no|never)\b", outcome_lower))

            return {
                "cause_event": cause_part,
                "outcome_text_raw": outcome_part,
                "outcome_direction": outcome_direction,
                "is_negated": "true" if is_negated else "false",
            }
...
""".strip()
        ...
        # Stage 1: prefer deterministic extraction; fall back to LLM when pattern doesn't match.
        python_stage1 = _python_stage1_extract(self.question)
        if python_stage1 is not None:
            data_1 = python_stage1
        else:
            # 调用 Stage 1
...
Medical note: Prefer clinically meaningful intermediate concepts (symptoms/signs, risk factors, exposures, pathophysiology, tests) over vague placeholders (e.g., "factor", "issue", "condition").
...
Domain note (clinical/medical):
- Prefer clinically meaningful intermediate variables (symptoms, signs, exposures, risk factors, tests, pathophysiology) that help reach TARGET_HINT (Y).
- Keep node text specific; avoid vague placeholders like "factor", "issue", "condition".
...
Domain note (clinical/medical):
- If Y is a diagnosis probability/risk, propose direct clinical causes/evidence (risk factors, symptoms, exposures, tests) rather than generic abstractions.
...
Medical note: In this dataset, Cause/Effect may be patient findings, exposures, risk factors, tests, treatments, or diagnosis probabilities.
...
""".strip()
    ...
    def _final_llm_decision(
        ...
        prompt = f"""
You are solving a medical causal reasoning problem (DDXPlus-style).
Your job is to decide how the CAUSE affects the BASE VARIABLE (a disease/outcome probability).
...
Domain notes (clinical/medical):
- Treat the BASE variable as a probability/risk/likelihood of a diagnosis or clinical outcome.
- Treat the CAUSE event as a clinical feature (symptom/sign), risk factor, exposure, history, test result, or treatment status.
- Many feature strings contain awkward grammar (e.g., "... or do the patient ..."). Treat the whole CAUSE string as ONE feature.
...
- You must decide the direction of change for the BASE VARIABLE only
  ...
  Ignore "more"/"less" in the question text; mapping back to the dataset's asked
  outcome ("more probability" vs "less probability") is handled outside of you.
  Example: if the question asks about "less probability of Disease" and you judge
  that Disease probability goes DOWN, output effect_on_base="less" (base decreases).
...
Medical commonsense priors (use when graph evidence is missing/weak/noisy):
- If the CAUSE describes presence of a symptom/sign that is typical for the diagnosis, the diagnosis probability usually INCREASES.
- If the CAUSE describes absence of a typical symptom/sign ("does not have/feel", "no", "never", "without"), the diagnosis probability usually DECREASES.
- If the CAUSE describes exposure/contact/travel, the relevant infectious diagnosis probability usually INCREASES; lack of exposure usually DECREASES it.
- If the CAUSE describes a risk factor/history (e.g., high cholesterol, smoking, family history), probability usually INCREASES; absence usually DECREASES it.
- If the CAUSE is phrased like "has CONDITION or takes medications to treat CONDITION", treat this as evidence the underlying CONDITION/risk factor is present (often increasing risk for related diseases).
- If the CAUSE is phrased like "not being treated for CONDITION", treat it as weak evidence the CONDITION is absent; lean toward DECREASING probability of complications linked to that CONDITION.
...
- "Smoking" -> INCREASE -> "Lung cancer risk": TRUE.
- "Vaccination" -> DECREASE -> "Infection probability": TRUE.
...
def main():
    # 示例数据点
    datapoint = {'question_stem': 'If the patient has had chills or shivers, how will this affect the more probability of Pneumonia?',
 'answer_label': 'more',
